...
@Repository
public class AluminiSendLinkToStudentDaoClass implements AluminiSendLinkToStudentDaoInterface{
	...
	public List<StudentAdmissionModel> getStudentInfo(String mailID) {
		...
		List<StudentAdmissionModel> StudentList=new ArrayList<>();
		
		SQLQuery query=this.sessionFactory.getCurrentSession().createSQLQuery("SELECT s.studentFirstName,s.studentLastName FROM studentadmission s where s.studentEmail='"+mailID+"' and s.status='Cancel'");
