...
<script type="text/javascript">
	$(document).ready(function() 
	{
		...
		$("#StreamID").change(function(){
			...
			$.getJSON('GetBranchListJsonStandardForExam.html',{id : id}).done(function(data) 
			...
			$.getJSON('GetStandardListJSONStandardForExam.html',{branchid : branchid}).done(function(data) {
				//alert(data[0].branchId);
			...
			$("#standardName").change(function(){
				...
				$("#StandardData").empty();
				$.getJSON('StudentStandardWiseRecordExam.html',{yearId:yearId,streamid:streamid,branchid : branchid,standardID:standardID}).done(function(data) {
					
					for (var i=0;i<data.length;i++)
					{
						
					$("#StudentData").append("<tr><td><input type='checkbox' name='StudID"+i+"' value='"+data[i].admissionRegId+"'  ></td><td>"+data[i].admissionRegId+"</td><td>"+data[i].studentFirstName +" "+data[i].studentLastName+ "</td><td>"+data[i].acadamicYear+"</td><td>"+data[i].streamName+"</td><td>"+data[i].branchName+"</td><td>"+data[i].standard+"</td><td>"+data[i].studentContactNumber+"</td></tr>");
...
<div class="page-content">
		...
		<form action="SendNoticeToStudentExam.html"  method="post" id="" class="form-horizontal">
		<div class="row">
			<div class="col-xs-12">
				<!-- PAGE CONTENT BEGINS -->
				...
				<div class="form-group">
						...
						<div class="col-sm-6">
							<textarea class="form-control" id="" rows="10"
								placeholder="Notice To Student" name="StudentExamNotice" required></textarea>
						</div>
					</div>
			</div>
		</div>
