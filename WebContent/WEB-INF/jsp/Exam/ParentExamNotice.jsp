...
<script type="text/javascript">
	$(document).ready(function() 
	{
		...
		$("#StreamID").change(function(){
			...
			$.getJSON('GetBranchListJsonStandardForYearParentExam.html',{id : id}).done(function(data) 
			...
			$.getJSON('GetStandardListJSONStandardForYearParentExam.html',{branchid : branchid}).done(function(data) {
				//alert(data[0].branchId);
			...
			$("#standardName").change(function(){
				...
				$("#StudentData").empty();
				$.getJSON('StudentAndParentRecordExam.html',{yearId:yearId,streamid:streamid,branchid : branchid,standardID:standardID}).done(function(data) {
					
					for (var i=0;i<data.length;i++)
					{
						
					$("#StudentData").append("<tr><td><input type='checkbox' name='StudID"+i+"' value='"+data[i].admissionRegId+"'  ></td><td>"+data[i].admissionRegId+"</td><td>"+data[i].studentFirstName +" "+data[i].studentLastName+ "</td><td>"+data[i].fatherFirstName +" "+data[i].fatherLastName+ "</td><td>"+data[i].fatherEmail+"</td><td>"+data[i].fatherContactNumber+"</td><td>"+data[i].acadamicYear+"</td></tr>");
...
<div class="page-content">
		<div class="page-header">
			<center>
				<h1><b>Message To Parent</b></h1>
			</center>
		</div>
		<form action="sendMessageToParentExam.html"  method="post" id="" class="form-horizontal">
		<div class="row">
			<div class="col-xs-12">
				<!-- PAGE CONTENT BEGINS -->
				...
				<div class="form-group">
						...
						<label class="col-sm-3 control-label no-padding-right">
						Exam Notice:<span style="color:red;">*</span></label>
						<div class="col-sm-6">
							<textarea class="form-control" id="" rows="10"
								placeholder="Message To Parent" name="ParentMessage" required></textarea>
						</div>
					</div>
			</div>
		</div>
				...
				<div class="row">
					<div class="col-xs-12">
						...
						<input type="hidden" id="Count" name="CheckCount">
							<button type="submit" class="btn btn-sm btn-success" name="MessageParent"
								id="SubmitCheck">
								Send... <i
