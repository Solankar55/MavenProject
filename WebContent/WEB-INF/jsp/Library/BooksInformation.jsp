...
$(document).ready(function(){
	...
	$('#subjectName').autocomplete({
		source : '${pageContext.request.contextPath}/searchSubName.html'
	});
	

	$('#titleName').autocomplete({
		source : '${pageContext.request.contextPath}/searchTitleName.html'
	});
	
	$('#authorName').autocomplete({
		source : '${pageContext.request.contextPath}/searchAuthorName.html'
	});
	 ...
	 $("#subjectName").blur(function(){
		...
		if(!subName=="" && titleName=="" && authorName=="")
		{
			...
			$("#searchButton").hide();
			$.getJSON('getSubjectList.html',{subjectName:subjectName}).done(function(data){
 	...
 	$("#titleName").blur(function(){
		...
		if(subName=="" && !titleName=="" && authorName=="")
		{
			...
			$("#searchButton").hide();
			$.getJSON('getTitleList.html',{titleName:titleName}).done(function(data){
				for(var i=0;i<data.length;i++)
					{
					$("#bookInformation").append("<tr><td>"+data[i].LibrarySubject+"</td><td>"+data[i].BookFor+"</td><td>"+data[i].Booktype+"</td><td>"+data[i].Title+"</td><td>"+data[i].Author+"</td><td>"+data[i].Edition+"</td><td>"+data[i].PublicationYear+"</td><td>"+data[i].Publisher+"</td><td>"+data[i].StockQuantity+"</td></tr>");
					}
 	...
 	$("#authorName").blur(function(){
		...
		if(subName=="" && titleName=="" && !authorName=="")
		{
			...
			$("#bookInformation").empty();
			//$("#authorName").val("");
			$.getJSON('getAuthorList.html',{authorName:authorName}).done(function(data){
				for(var i=0;i<data.length;i++)
					{
					$("#bookInformation").append("<tr><td>"+data[i].LibrarySubject+"</td><td>"+data[i].BookFor+"</td><td>"+data[i].Booktype+"</td><td>"+data[i].Title+"</td><td>"+data[i].Author+"</td><td>"+data[i].Edition+"</td><td>"+data[i].PublicationYear+"</td><td>"+data[i].Publisher+"</td><td>"+data[i].StockQuantity+"</td></tr>");
					}
		 ...
		 if(subName.length!=0  && titleName.length!=0 && authorName.length==0)
			{
				...
				$.getJSON('getBooksubTitleList.html',{subName:subName,titleName:titleName,authorName:authorName}).done(function(data){
					for (var i=0;i<data.length;i++)
						{
			...
			$.getJSON('getBookTitleAuthorList.html',{subName:subName,titleName:titleName,authorName:authorName}).done(function(data){
				for(var i=0;i<data.length;i++)
					{
					$("#bookInformation").append("<tr><td>"+data[i].LibrarySubject+"</td><td>"+data[i].BookFor+"</td><td>"+data[i].Booktype+"</td><td>"+data[i].Title+"</td><td>"+data[i].Author+"</td><td>"+data[i].Edition+"</td><td>"+data[i].PublicationYear+"</td><td>"+data[i].Publisher+"</td><td>"+data[i].StockQuantity+"</td></tr>");
					}
			...
			$.getJSON('getBookSubAuthorList.html',{subName:subName,titleName:titleName,authorName:authorName}).done(function(data){
				for (var i=0;i<data.length;i++)
					{
			...
			$.getJSON('getBookDetailList.html',{subName:subName,titleName:titleName,authorName:authorName}).done(function(data){
				for (var i=0;i<data.length;i++)
					{
		...
		<div class="page-header center">
			...
			<div id="basicFormExample" class="panel-collapse collapse in">
				...
				<form class="form-inline" action=" ">
					...
					<div class="form-group">
						<label for="searchPublisher"><b>Publisher </b> </label><input type="text"
							class="form-control"  id="publisherName"
							name="publisherName" placeholder="Enter Correct Publisher-Name" class="cat"  />
					</div>
